...
async function getManifestURLs(limit = 10, offset = 0) {
  ...
  const querySources = ['https://stad.gent/sparql'];
  ...
  return await comunicaEngine.queryBindings(query, { sources: querySources });
}

async function getManifestCanvasses(manifestURL, limit = 10, offset = 0) {
  const comunicaEngine = new QueryEngine();
...
async function workflow() {
  ...
  workflow.on(exception, (error) => {
    console.error(error);
  });
  workflow.on(success, (response) => {
    console.log(response);
  });
  workflow.on(newManifest, async (url) => {

    // Get manifest canvasses
    const canvasStream = await getManifestCanvasses(url, 1);
    canvasStream.on('data', (canvas) => {
  ...
  manifestStream.on('data', (manifest) => {
